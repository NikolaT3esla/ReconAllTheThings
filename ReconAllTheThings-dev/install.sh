...
printf "\n\n${bgreen}#######################################################################${reset}\n"
printf "${bgreen} reconAllTheThings installer/updater script ${reset}\n\n"
...
printf "${bblue} Running: Looking for new reconAllTheThings version${reset}\n\n"
...
then
    ...
    if git status --porcelain | grep -q 'reconAllTheThings.cfg$'; then
        mv reconAllTheThings.cfg reconAllTheThings.cfg_bck
        printf "${yellow} reconAllTheThings.cfg has been backed up in reconAllTheThings.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} reconAllTheThings is already up to date!${reset}\n\n"
fi
...
EOF
    ...
    printf "${byellow} Golang will not be configured according to the user's prefereneces (reconAllTheThings.cfg install_golang var)${reset}\n";
fi
